...
/// A running GPUI application that was started in "embedded" mode.
///
/// In embedded mode, GPUI does not attempt to take ownership of the platform event loop.
/// This is intended for host applications (such as Electron) that already created and
/// are already pumping the OS UI loop.
///
/// Note that some platform integrations (e.g. macOS app-delegate callbacks) may require
/// additional host cooperation and/or platform-specific embedding support.
#[derive(Clone)]
pub struct EmbeddedApplication(Rc<AppCell>);
...
impl Application {
    ...
    /// Start the application in embedded mode.
    ///
    /// This method does **not** start (or take ownership of) any platform event loop. Instead, it
    /// invokes the provided callback immediately on the current thread, and returns a handle that
    /// keeps the application alive for as long as it is held.
    ///
    /// This is intended for host applications that already created and are already pumping the OS
    /// UI loop (e.g. an Electron app). The embedded application must be created and used on the
    /// platform's UI thread.
    pub fn run_embedded<F>(self, on_finish_launching: F) -> EmbeddedApplication
    where
        F: 'static + FnOnce(&mut App),
    {
        let this = self.0.clone();
        let platform = { self.0.borrow().platform.clone() };
        platform.run_embedded(Box::new(move || {
            let cx = &mut *this.borrow_mut();
            on_finish_launching(cx);
        }));

        EmbeddedApplication(self.0)
    }
