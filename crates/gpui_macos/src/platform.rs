...
unsafe fn get_associated_id(object: id, key: &'static u8) -> id {
    unsafe { objc_getAssociatedObject(object, key as *const _ as *const c_void) }
}
    ...
    fn run_embedded(&self, on_finish_launching: Box<dyn FnOnce()>) {
        ...
        unsafe {
            ...
            let proxy = if let Some(proxy) = state.embedded_delegate_proxy {
                ...
                let current_class: *const Class = if current_delegate.is_null() {
                    ptr::null()
                } else {
                    msg_send![current_delegate, class]
                };
                if !current_delegate.is_null() && current_class == APP_DELEGATE_PROXY_CLASS {
                    ...
                    state.embedded_original_delegate =
                        (!current_delegate.is_null()).then_some(current_delegate);
            ...
            let proxy_object: &mut Object = &mut *proxy;
...
impl Drop for MacPlatform {
    fn drop(&mut self) {
        unsafe {
            ...
            let state = self.0.lock();
            if let Some(proxy) = state.embedded_delegate_proxy {
                set_associated_id(proxy, &MAC_PLATFORM_ASSOC_KEY, nil);
                if let Some(original) = state.embedded_original_delegate {
                    app.setDelegate_(original);
                }
            }
        }
    }
...
extern "C" fn will_finish_launching(this: &mut Object, _: Sel, notification: id) {
    unsafe {
        ...
        if original != nil {
            let responds: bool =
                msg_send![original, respondsToSelector: sel!(applicationWillFinishLaunching:)];
            if responds {
...
extern "C" fn did_finish_launching(this: &mut Object, _: Sel, notification: id) {
    unsafe {
        ...
        if original != nil {
            let responds: bool =
                msg_send![original, respondsToSelector: sel!(applicationDidFinishLaunching:)];
            if responds {
...
extern "C" fn will_terminate(this: &mut Object, _: Sel, notification: id) {
    unsafe {
        ...
        if original != nil {
            let responds: bool =
                msg_send![original, respondsToSelector: sel!(applicationWillTerminate:)];
            if responds {
...
extern "C" fn open_urls(this: &mut Object, _: Sel, _: id, urls: id) {
    ...
    unsafe {
        if original != nil {
            let responds: bool =
                msg_send![original, respondsToSelector: sel!(application:openURLs:)];
            if responds {
                let app = NSApplication::sharedApplication(nil);
...
extern "C" fn validate_menu_item(this: &mut Object, _: Sel, item: id) -> bool {
    unsafe {
        ...
        if !is_gpui_action {
            ...
            if original != nil {
                let responds: bool =
                    msg_send![original, respondsToSelector: sel!(validateMenuItem:)];
                if responds {
...
extern "C" fn handle_dock_menu(this: &mut Object, _: Sel, _: id) -> id {
    unsafe {
        ...
        if original != nil {
            let responds: bool =
                msg_send![original, respondsToSelector: sel!(applicationDockMenu:)];
            if responds {
                let app = NSApplication::sharedApplication(nil);
